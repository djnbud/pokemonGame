...
function pokemonBagInit() {
  ...
  createBackButton(
    "pokemonBagBattleBack",
    "pokemonBattleBackDiv",
    "pokemonBagUIBattle",
    "inventoryUI"
  );
  document
    .querySelector("#pokemonBagBattleBack")
  ...
  queue.push(() => {
    ...
    if (playerPokemon.health <= 0) {
      ...
      if (checkAllLocalPokemonHealth()) {
        ...
        document.querySelector("#pokemonBagUIBattle").style.visibility =
          "visible";
        ...
        queue.push(() => {
          document.querySelector("#dialogueBox").innerHTML =
            "You have no Pokemon Left and have had to flee!";
        });
        healAllLocalPokemon();
        endBattle(false);
      }
...
function prepareBattle(pokeInd, enemyPoke) {
  ...
  if (playerPokemonDetails.details.health > 0) {
    ...
    let healthPercentage =
      (playerPokemon.health / playerPokemon.maxHealth) * 100;
    ...
    let nextLevelNeeded =
      experienceLevelGrid[nextLvl][playerPokemon.levelingType];
    let expPercentage =
      (playerPokemonDetails.details.experience / nextLevelNeeded) * 100;
    ...
    let healthEnemyPercentage =
      (enemyPokemon.health / enemyPokemon.maxHealth) * 100;
    gsap.to("#enemyHealthBar", {
      width: healthEnemyPercentage + "%",
    });

    document.querySelector("#enemyPokemonName").innerHTML =
      monsters[enemyPoke].name;
    document.querySelector("#enemyPokemonLevel").innerHTML =
      monsters[enemyPoke].level;
    document.querySelector("#playerPokemonName").innerHTML =
      playerPokemonDetails.nickname;
    document.querySelector("#playerPokemonLevel").innerHTML =
      playerPokemonDetails.details.level;
    ...
    document.querySelector("#dialogueBox").style.display = "block";
    document.querySelector("#dialogueBox").innerHTML =
      "A wild " + enemyPokemonSpec.name + " appeared!";
    ...
    onComplete: () => {
      if (widthAmount < 100) {
        ...
        gsap.to("#PlayerExpBar", {
          ...
          onComplete: () => {
            document.querySelector("#playerPokemonLevel").innerHTML =
              playerPokemon.level + levelCount;

            levelCount++;
            exp -= nextLevelNeeded;
            let playerNextLevel = playerPokemon.level + levelCount;
            nextLevelNeeded =
              experienceLevelGrid[playerNextLevel][playerPokemon.levelingType];
...
function finishExpGain(exp, levelCount) {
  ...
  if (newLevel > playerPokemon.level) {
    document.querySelector("#dialogueBox").innerHTML =
      playerPokemon.name + " reached level " + newLevel;
  }
  let localPoke = getLocalStoredPokemon(),
    playerPokemonDetails = localPoke.get(currentSelectedPokemonIndex);
      ...
      onComplete: () => {
        gsap.to(postEvolution, {
          ...
          onComplete: () => {
            ...
            let evolvedPokemonStats = statCalculator(
              playerPokemonDetails.details,
              newPokemonSpec
            );
            playerPokemonDetails.details.maxHealth =
              evolvedPokemonStats.newHealth;
            playerPokemonDetails.details.attackStat =
              evolvedPokemonStats.newAttack;
            playerPokemonDetails.details.defenseStat =
              evolvedPokemonStats.newDefense;
            playerPokemonDetails.details.speedStat =
              evolvedPokemonStats.newSpeed;
            playerPokemonDetails.details.experience = exp;
...
function experienceGained() {
  ...
  document.querySelector("#dialogueBox").innerHTML =
    playerPokemon.name + " gained " + newExp + " experience!";
  let nextLvl = playerPokemon.level + 1;
  let nextLevelNeeded =
    experienceLevelGrid[nextLvl][playerPokemon.levelingType];
...
function addAttackQuery(id) {
  ...
  document.querySelector(id).addEventListener("mouseenter", (e) => {
    ...
    document.querySelector("#attackDamage").innerHTML =
      "Attack Damage: " + selectedAttack.damage;
  });
}
...
function addPokeballQuery(id) {
  document.querySelector(id).addEventListener("click", (e) => {
    ...
    if (curentPokeball.amount > 0) {
      ...
      document.querySelector("#dialogueBox").style.display = "block";
      document.querySelector("#dialogueBox").innerHTML =
        "You throw a Pokeball at " + enemyPokemon.name;
      queue.push(() => {
      ...
      if (id !== "#pokemonBagBattleBtn" + currentSelectedPokemonIndex) {
        ...
        if (playerPokemonDetails.details.health > 0) {
          document.querySelector(
            "#pokemonBagBattleBtn" + currentSelectedPokemonIndex
...
document.querySelector("#runAway").addEventListener("click", (e) => {
  if (playerPokemon.runAwayAttempt()) {
    queue.push(() => {
      document.querySelector("#dialogueBox").style.display = "block";
      document.querySelector("#dialogueBox").innerHTML =
        playerPokemon.name + " successfully escaped";
    });
    endBattle(false);
  } else {
    queue.push(() => {
      document.querySelector("#dialogueBox").style.display = "block";
      document.querySelector("#dialogueBox").innerHTML =
        playerPokemon.name + " failed to escape";
    });
    queue.push(() => {
      enemyAttacks();
    });
